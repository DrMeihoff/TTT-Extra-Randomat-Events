CreateConVar("randomat_monkey_mult", 1.25, {FCVAR_NOTIFY, FCVAR_ARCHIVE}, "Jump height multiplier", 1, 5)

local EVENT = {}
EVENT.Title = "monkey"
EVENT.Description = "You can jump much higher now."
EVENT.id = "monkey"
...
---- Commands to add to Randomat ULX command table --------------------------------
	...
	events["monkey"].commands = { 						-- New section to allow for transfer of commands easily
    "ttt_randomat_monkey",
	}
...
---- Check for Data File and our event --------------------------------------------
    ...
    if not data then 										-- We do this here to check if we are the first addon
        ...
        print("[RANDOMAT] [monkey] No randomataddons file found, creating one")
    end 

    data = file.Read( "randomataddons.txt", "DATA" ) 		-- File still doesnt exist, must be an issue...
    if not data then return end

    print("[RANDOMAT] [monkey] Found randomataddons.txt")
    ...
    for _, line in ipairs( tabdata ) do						-- However we need to check if our addon already exists in file
        if line .. "\n" == eventJSON then					
            print("[RANDOMAT] [monkey] Event found in randomataddons list")
            alreadyExists = true
        end
    end
    if (!alreadyExists) then								-- JSON Table not found in file, writing to file
        print("[RANDOMAT] [monkey] Did not find event in list, adding")			-- PLS NOTE, if the table is altered it will add the entry again
...
function EVENT:Begin()
    for i, v in ipairs( player.GetAll() ) do
		v:SetJumpPower(200 * GetConVar("randomat_monkey_mult"):GetFloat())
    end
end
