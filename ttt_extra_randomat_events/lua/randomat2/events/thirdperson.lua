...
---- Define Randomat ULX Table Entries --------------------------------------------
	...
	events["thirdperson"] = {}
    events["thirdperson"].name = "thirdperson"
...
---- Commands to add to Randomat ULX command table --------------------------------
	...
	events["thirdperson"].commands = { 						-- New section to allow for transfer of commands easily
    "ttt_randomat_thirdperson",
	}
...
---- Check for Data File and our event --------------------------------------------
    ...
    if not data then 										-- We do this here to check if we are the first addon
        ...
        print("[RANDOMAT] [THIRDPERSON] No randomataddons file found, creating one")
    end 

    data = file.Read( "randomataddons.txt", "DATA" ) 		-- File still doesnt exist, must be an issue...
    if not data then return end

    print("[RANDOMAT] [THIRDPERSON] Found randomataddons.txt")
    ...
    for _, line in ipairs( tabdata ) do						-- However we need to check if our addon already exists in file
        if line .. "\n" == eventJSON then					
            print("[RANDOMAT] [THIRDPERSON] Event found in randomataddons list")
            alreadyExists = true
        end
    end
    if (!alreadyExists) then								-- JSON Table not found in file, writing to file
        print("[RANDOMAT] [THIRDPERSON] Did not find event in list, adding")			-- PLS NOTE, if the table is altered it will add the entry again
